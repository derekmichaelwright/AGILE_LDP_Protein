...
> - `r shiny::icon("newspaper")` [Derek Wright, Jiayi Hang, James D House & Kirstin E Bett. **Evaluating the breeding potential of cultivated lentils for protein and amino acid concentration and quality**. *unpublished*](https://github.com/derekmichaelwright/AGILE_LDP_Protein)
...
DT_AA_Selection <- function(myAmino = "Protein") {
  ...
  ss <- xx %>% select(Entry, Name, Origin, DTF_Cluster, STR_Group, ExptShort, Selection) %>% 
    ...
    filter(!is.na(Ro16), !is.na(Ro17), !is.na(Su16), !is.na(Su17)) %>% # must be "Yes" in all Expt
    mutate(Trait = myAmino) %>% select(Entry, Name, Origin, DTF_Cluster, STR_Group, Trait)
}
#
...
ggsave("Additional/Additional_Figure_04.png", mp, width = 10, height = 8, dpi = 600)
```

---

## Additional Figure 5 - Structure Groups

> - `r shiny::icon("chart-line")` [Additional/Additional_Figure_05.html](https://derekmichaelwright.github.io/AGILE_LDP_Protein/Additional/Additional_Figure_05.html)
...
# Prep data
myCots <- c("Red", "Yellow")
#mySTR <- read.csv("Data/myD_LDP_Str.csv") %>% select(Entry, STR_Group)
xx <- d2 %>% 
  filter(AminoAcid == "Protein") %>%
  left_join(myLDP, by = c("Entry", "Name")) %>%
  #left_join(mySTR, by = "Entry") %>%
  filter(CotyledonColor %in% myCots) %>%
  mutate(CotyledonColor = factor(CotyledonColor, levels = myCots))
# Plot
mp <- ggplot(xx, aes(x = STR_Group, y = Value, color = CotyledonColor, key1 = Origin)) +
  ...
  labs(title = "A) Protein Concentration by Structure Group", 
       x = NULL, y = "g / 100g dry seed")
ggsave("Additional/Additional_Figure_05.png", mp, width = 8, height = 6, dpi = 600)
#
saveWidget(ggplotly(mp), file="Additional/Additional_Figure_05.html")
```

---

## Additional Figure 6 - Size
