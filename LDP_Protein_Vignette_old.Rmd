...
myEAA <- myEAA <- c("Histidine","Isoleucine","Leucine","Lysine","Methionine","Phenylalanine","Threonine","Tryptophan","Valine")
...
myY <- d4 %>% 
  select(-Expt, -Protein) %>% 
  gather(Trait, Value, Essential.AA, Perc.Essential.AA) %>%
  mutate(Trait = paste(Trait, ExptShort, sep = "_")) %>%
  select(-ExptShort) %>%
  spread(Trait, Value) %>%
  fixNames()
write.csv(myY, "Data/myY_NIRS_EAA.csv", row.names = F)
```
...
```{r}
# Run GWAS on all data
myY <- read.csv("Data/myY_NIRS_EAA.csv")
myGAPIT <- GAPIT(
  Y = myY,
  G = myG,
  PCA.total = 4,
...
#myG <- read.table("Data/myG_LDP_RD5_MAF5_MISS10.txt", header = T)
```
...
```{r}
myR3 <- list_Result_Files("new_GWAS_Results_EAA/")
order_GWAS_Results(folder = "new_GWAS_Results_EAA/", files = myR3)
#
yy <- table_Results_Summary("new_GWAS_Results_EAA/", isOrdered = F)
yy <- table_Results_Summary("new_GWAS_Results_EAA/", isOrdered = T)
#
myR3 <- list_Result_Files("GWAS_Results_DTF_REP_2/")
order_GWAS_Results(folder = "GWAS_Results_DTF_REP_2/", files = myR3)
#
yy <- table_Results_Summary(folder = "GWAS_Results_DTF_REP_2/", isOrdered = F)
yy <- table_Results_Summary(folder = "GWAS_Results_DTF_REP_2/", isOrdered = T)
#
nrow(xx %>% filter(Threshold == "Significant") %>% filter(!duplicated(SNP)))
```

```{r eval = F, echo = F}
yy <- table_Results_Summary("new_GWAS_Results/", isOrdered = F)
yy <- table_Results_Summary("new_GWAS_Results/", isOrdered = T)
yy <- table_Results_Summary(folder = "new_GWAS_Results_DTF_REP/", isOrdered = F)
yy <- table_Results_Summary(folder = "new_GWAS_Results_DTF_REP/", isOrdered = T)
nrow(xx %>% filter(Threshold == "Significant") %>% filter(!duplicated(SNP)))
```

```{r}
myR3 <- list_Result_Files("GWAS_Results_EA/")
order_GWAS_Results(folder = "GWAS_Results_EA/", files = myR3)
#
yy <- table_Results_Summary("GWAS_Results_EA/", isOrdered = F)
yy <- table_Results_Summary("GWAS_Results_EA/", isOrdered = T)
#
myR3 <- list_Result_Files("GWAS_Results_DTF_REP_2/")
order_GWAS_Results(folder = "GWAS_Results_DTF_REP_2/", files = myR3)
#
...
```{r eval = F}
# Create custom manhattan plots
myTs <- list_Traits("GWAS_Results_EA/")
for(i in myTs) {
  mp <- gg_Manhattan(folder = "GWAS_Results_EA/", trait = i, facet = F,
                     models = c("MLM", "MLMM", "FarmCPU", "BLINK"),
                     vlines = myMs, vline.colors = myCs,
                     threshold = 6.7, sug.threshold = 5, pmax = 12 )
  ggsave(paste0("Additional/ManH2/Multi_",i,"_1_noCV.png"), 
         mp, width = 12, height = 4, bg = "white")
}
#
myTs <- list_Traits("GWAS_Results_DTF_REP_2/")
for(i in myTs) {
  mp <- gg_Manhattan(folder = "GWAS_Results_DTF_REP_2/", trait = i, facet = F,
                     title = paste(i, "| CV = DTF + REP"),
                     models = c("MLM", "FarmCPU", "BLINK"),
                     vlines = myMs, vline.colors = myCs,
                     threshold = 6.7, sug.threshold = 5, pmax = 12 )
  ggsave(paste0("Additional/ManH2/Multi_",i,"_2_CVDTFREP.png"), 
         mp, width = 12, height = 4, bg = "white")
}
```
...
# Plot
mp4 <- ggplot(xx, aes(x = ExptShort, y = Protein)) +
  geom_violin() +
...
mp5 <- ggplot(xx, aes(x = ExptShort, y = Ratio)) +
  geom_violin() +
  geom_quasirandom(aes(color = ExptShort), pch = 16, size = 1, alpha = 0.7) +
  geom_boxplot(width = 0.1, coef = 5) +
  scale_color_manual(values = myCs_Expt) +
...
#
mp <- ggarrange(ggarrange(mp1, mp2, mp4, mp5, ncol = 2, nrow = 2), 
                mpl, nrow = 2, heights = c(1,0.1))
#mp <- ggarrange(mp1, ggarrange(mp2, ggarrange(mp4, mp5, ncol = 1), ncol = 2), mpl, nrow = 3, heights = c(1,1,0.1))
...
# Plot
mp <- ggplot(xx, aes(x = Name, y = Value, color = ExptShort)) +
...
ggsave("Figure_00_5.png", mp, width = 6, height = 4, dpi = 600, bg = "white")
```
...
x2 <- d2 %>% filter(AminoAcid %in% myEAA) %>%
  ...
  summarise(Essential = sum(Value)) %>% ungroup()
xx <- left_join(x1, x2, by = c("Name", "ExptShort")) %>% 
  mutate(Ratio = Essential/Protein)
# Plot
mp <- ggplot(xx, aes(x = ExptShort, y = Protein)) +
  geom_violin() +
  geom_quasirandom(aes(color = ExptShort), alpha = 0.7) +
  geom_boxplot(width = 0.1, coef = 5) +
  #geom_line(aes(group = Name), alpha = 0.2) +
  scale_color_manual(values = myCs_Expt) +
  theme_AGL +
  labs(title = "Total Protein")
ggsave("Figure_00_1.png", mp, width = 6, height = 4, dpi = 600, bg = "white")
mp <- ggplot(xx, aes(x = ExptShort, y = Essential)) +
  geom_violin() +
  geom_quasirandom(aes(color = ExptShort), alpha = 0.7) +
  geom_boxplot(width = 0.1, coef = 5) +
  #geom_line(aes(group = Name), alpha = 0.2) +
  scale_color_manual(values = myCs_Expt) +
  theme_AGL +
  labs(title = "Essential AA")
ggsave("Figure_00_2.png", mp, width = 6, height = 4, dpi = 600, bg = "white")
mp <- ggplot(xx, aes(x = ExptShort, y = Ratio)) +
  geom_violin() +
  geom_quasirandom(aes(color = ExptShort), alpha = 0.7) +
  geom_boxplot(width = 0.1, coef = 5) +
  #geom_line(aes(group = Name), alpha = 0.2) +
  scale_color_manual(values = myCs_Expt) +
  theme_AGL +
  labs(title = "Essential AA / Total Protein")
ggsave("Figure_00_3.png", mp, width = 6, height = 4, dpi = 600, bg = "white")
mp <- ggplot(xx, aes(x = ExptShort, y = Ratio)) +
  geom_line(aes(group = Name), alpha = 0.3) +
  theme_AGL +
  labs(title = "Essential AA / Total Protein")
ggsave("Figure_00_4.png", mp, width = 6, height = 4, dpi = 600, bg = "white")
```

```{r}
library(lme4)
xx <- d2 %>% filter(AminoAcid == "Protein")
model <- lmer(Value ~ Name + (1|Location) + (1|Year), data = xx)
#model <- lm(Value ~ Name + Location + Year + (1|Name*Location + Name*Year + Name*Location*Year, data = xx)
model
```
...
mp1 <- gg_Selection(myExpt = "Su16", myColor = "steelblue",
                    myTitle ="A) Sutherland, Canada 2016")
mp2 <- gg_Selection(myExpt = "Su17", myColor = "darkblue",
                    myTitle ="B) Sutherland, Canada 2017")
mp3 <- gg_Selection(myExpt = "Ro16", myColor = "darkorange",
                    myTitle ="C) Rosthern, Canada 2016")
mp4 <- gg_Selection(myExpt = "Ro17", myColor = "darkred",
...
DT_AA_Selection <- function(myAmino = "Protein") {
  ...
  ss <- xx %>% select(Entry, Name, ExptShort, Selection) %>% spread(ExptShort, Selection) %>% 
    filter(!is.na(Ro16), !is.na(Ro17), !is.na(Su16), !is.na(Su17)) %>% # must be "Yes" in all Expt
...
```{r}
gg_GWAS_Summary <- function (folder = NULL, traits = list_Traits(), threshold = -log10(5e-08), 
    sug.threshold = -log10(5e-06), models = c("MLM", "MLMM", 
        "FarmCPU", "BLINK", "GLM"), colors = c("darkgreen", "darkred", 
        "darkorange3", "steelblue", "darkgoldenrod2"), shapes = 21:25, 
    hlines = NULL, vlines = NULL, vline.colors = rep("red", length(vlines)), 
    vline.legend = T, title = NULL, caption = paste0("Sig Threshold = ", 
        threshold, " = Large\nSuggestive = ", sug.threshold, 
        " = Small"), rowread = 2000, legend.position = "bottom", 
    lrows = 1) 
{
    ...
    for (i in files) {
        ...
        if (nrow(myPi) > 0) {
            myP <- bind_rows(myP, myPi)
        }
    }
    myP <- myP %>% filter(!is.na(SNP)) %>% arrange(Chr, Pos, 
        P.value, Trait) %>% mutate(Model = factor(Model, levels = models), 
        Trait = factor(Trait, levels = rev(traits))) %>% filter(!is.na(Trait)) %>% 
        arrange(desc(Model))
    ...
    myG <- read.csv(paste0(folder, files[1])) %>% mutate(Trait = myP$Trait[1], 
      ...
      filter(Chr %in% c("Lcu.1GRN.Chr1","Lcu.1GRN.Chr2","Lcu.1GRN.Chr3","Lcu.1GRN.Chr4","Lcu.1GRN.Chr5","Lcu.1GRN.Chr6","Lcu.1GRN.Chr7"))
    mp <- ggplot(x1, aes(x = Pos/1e+08, y = Trait)) + geom_blank(data = myG)
    if (!is.null(vlines)) {
        myGM <- myG %>% filter(SNP %in% vlines) %>% mutate(SNP = factor(SNP, 
            levels = vlines)) %>% arrange(SNP)
        mp <- mp + geom_vline(data = myGM, alpha = 0.5, aes(xintercept = Pos/1e+08, 
            color = SNP)) + scale_color_manual(name = NULL, values = vline.colors)
    }
    if (!is.null(hlines)) {
        mp <- mp + geom_hline(yintercept = hlines, alpha = 0.7)
    }
    
    mp <- mp + geom_point(data = x2, size = 0.75, color = "black", 
        alpha = 0.5, aes(shape = Model, fill = Model, key1 = SNP, 
            key2 = `-log10(p)`)) + geom_point(size = 2.25, color = "black", 
        alpha = 0.5, aes(shape = Model, fill = Model, key1 = SNP, 
            key2 = `-log10(p)`)) + facet_grid(. ~ Chr, drop = F, 
        scales = "free_x", space = "free_x") + scale_fill_manual(name = NULL, 
        values = colors, breaks = models) + scale_shape_manual(name = NULL, 
        values = shapes, breaks = models) + scale_x_continuous(breaks = 0:20) + 
        scale_y_discrete(drop = F) + theme_gwaspr(legend.position = legend.position) + 
        guides(shape = guide_legend(nrow = lrows, override.aes = list(size = 4)), 
            color = guide_legend(nrow = lrows), fill = guide_legend(nrow = lrows)) + 
...
```{r}
# Prep data
myMs <- c("Lcu.1GRN.Chr3p339861922", "Lcu.1GRN.Chr5p330972006", "Lcu.1GRN.Chr5p466297891",
          "Lcu.1GRN.Chr1p437956256", "Lcu.1GRN.Chr4p429808510", "Lcu.1GRN.Chr6p411147054")
 
```

```{r}
# Prep data
myPs <- c("Essential.AA", "Perc.Essential.AA","Protein", "Glutamate", "Aspartate", "Leucine",
...
ggsave("Additional/Test_Figure_04d.png", mp, width = 12, height = 12, bg = "white")
# Save HTML
mp <- ggplotly(mp) %>% layout(showlegend = FALSE)
saveWidget(as_widget(mp), "Additional/Test_Figure_04_d.html",
...
myTs <- c(paste0(myPs, "_Su16"), paste0(myPs, "_Su17"),
          paste0(myPs, "_Ro16"), paste0(myPs, "_Ro17"))
# Plot
mp1 <- gg_GWAS_Summary(folder = "old_GWAS_Results_DTF_REP/", 
                       traits = myTs,
                       models = c("MLM", "MLMM", "FarmCPU", "BLINK"),
                       colors = c("darkgreen", "darkred", "darkorange3", "steelblue"),
                       threshold = 6.7, sug.threshold = 5, 
  ...
  guides(fill = guide_legend(title="Model", order = 1, nrow = 2), 
         ...
         color = guide_legend(title="Marker", order = 2, byrow = T))
#
mp2 <- gg_GWAS_Summary(folder = "new_GWAS_Results_DTF_REP/", 
                       traits = myTs,
                       models = c("MLM", "MLMM", "FarmCPU", "BLINK"),
                       colors = c("darkgreen", "darkred", "darkorange3", "steelblue"),
                       threshold = 6.7, sug.threshold = 5, 
...
ggsave("Additional/Test_Figure_04a.png", mp, width = 12, height = 20, bg = "white")
# Save HTML
mp1 <- ggplotly(mp1) %>% layout(showlegend = FALSE)
saveWidget(as_widget(mp1), "Additional/Test_Figure_04_A.html",
           knitrOptions = list(fig.width = 12, fig.height = 10), 
           selfcontained = T)
mp2 <- ggplotly(mp2) %>% layout(showlegend = FALSE)
saveWidget(as_widget(mp2), "Additional/Test_Figure_04_B.html",
           knitrOptions = list(fig.width = 12, fig.height = 10), 
           selfcontained = T)

## Prep data
myPs <- c("Essential.AA", "Perc.Essential.AA")
#
myTs <- c(paste0(myPs, "_Su16"), paste0(myPs, "_Su17"),
          paste0(myPs, "_Ro16"), paste0(myPs, "_Ro17"))
# Plot
mp <- gg_GWAS_Summary(folder = "new_GWAS_Results_EAA/", 
                       traits = myTs,
                       models = c("MLM", "MLMM", "FarmCPU", "BLINK"),
                       colors = c("darkgreen", "darkred", "darkorange3", "steelblue"),
                       threshold = 6.7, sug.threshold = 5, 
  ...
  guides(fill = guide_legend(title="Model", order = 1, nrow = 2), 
         shape = guide_legend(title="Model", order = 1, nrow = 2),
         color = guide_legend(title="Marker", order = 2, byrow = T))
ggsave("Additional/Test_Figure_04b.png", mp, width = 12, height = 8, bg = "white")
# Save HTML
mp <- ggplotly(mp) %>% layout(showlegend = FALSE)
saveWidget(as_widget(mp), "Additional/Test_Figure_04_C.html",
           knitrOptions = list(fig.width = 12, fig.height = 10), 
           selfcontained = T)
```
...
myMs <- c("Lcu.1GRN.Chr3p339861922", "Lcu.1GRN.Chr5p330972006", "Lcu.1GRN.Chr5p466297891",
          "Lcu.1GRN.Chr1p437956256", "Lcu.1GRN.Chr4p429808510", "Lcu.1GRN.Chr6p411147054")
myMs <- c("Lcu.1GRN.Chr2p420147251", "Lcu.1GRN.Chr5p39874149", "Lcu.1GRN.Chr5p496452950",
          "Lcu.1GRN.Chr2p132574250", "Lcu.1GRN.Chr6p424934202", "Lcu.1GRN.Chr7p2703522")#"Lcu.1GRN.Chr7p644435786" "Lcu.1GRN.Chr3p1141893"
myY_Su17 <- read.csv("Data/myY_NIRS_Su17.csv")
myY_Su16 <- read.csv("Data/myY_NIRS_Su16.csv")
myY_Ro16 <- read.csv("Data/myY_NIRS_Ro16.csv")
myY_Ro17 <- read.csv("Data/myY_NIRS_Ro17.csv")
...
ggMultiMarkers <- function (myG, myY, myTrait, myMarkers, points = T, myColor = "steelblue",
    ...
    for(i in 12:ncol(x1)) { if(sum(!grepl("GG|CC|AA|TT", x1[,i])) > 0) { x1[,i] <- NA } }
    x1 <- x1[,!is.na(x1[1,])]
    #
...
ggMASMarkers <- function(myExpt, myColor) {
  ...
  #
  for(i in 12:ncol(xx)) { xx[,i] <- ifelse(!grepl("GG|CC|AA|TT", xx[,i]), NA, xx[,i]) }
  #
